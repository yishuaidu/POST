%streaming variational inference for continous tensor data
%prior_u = {prior_mean, prior_cov}
%prior_mean = {d_1 x R, ..., d_K x R}
%prior_post = {R x R x d_1, ..., R x R x d_K}
function post = POST_cont(prior, batch, opt)
    prior_mean = prior{1}; 
    prior_cov = prior{2};
    prior_a = prior{3}(1);
    prior_b = prior{3}(2);
    ...
    for iter = 1:opt.max_iter
        ...
        for k=1:nmod            
            ...
            for j=1:length(uind{k})
                uid = uind{k}(j);
                eid = data_ind{k}{j};
                post_cov{k}(:, :, uid) = inv(inv(prior_cov{k}(:,:, uid)) + tau*sum(t_nk2(:,:,eid), 3));
            ...
            a = prior_a + 0.5*n;
            b = prior_b + 0.5*sum(y.*y) - sum(t_k.*y) + 0.5*sum(t_k2(:)); 
            tau = a/b;
        end
        diff = 0;
        for k=1:nmod
